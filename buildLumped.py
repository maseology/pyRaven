...
from pyRaven import buildHBV, buildHMETS, buildGR4J, buildBlended, rvt_lumped, ostrich_HBV, ostrich_HMETS, ostrich_GR4J, ostrich_Blended, parameters
from pyRaven.flags import flg


def __lumpedCollect(ins, stmsg):
    desc = ins.desc
    print("\n" + "="*len(stmsg))
    print(stmsg)
    print("="*len(stmsg) + "\n")
    ...
    return root0, root, nam, desc, builder, ver, wshd, hrus, params, ts, dtb, dte, dftem
...
def GR4J(ins):
    b0 = timer()
    root0, root, nam, desc, builder, ver, wshd, _, params, ts, dtb, dte, dftem = __lumpedCollect(ins,  "=== Raven GR4J builder ===")
    ...
    if flg.calibrationmode: 
        # print('\n copying Ostrich templates..')
        ostrich_GR4J.writeDDS(root0 + nam + ins.sfx + "_CALIB\\", nam, 4000)
...
# the "blended" Raven model (as built by ORMGP for application in southern Ontario, Canada)
...
def Blended(ins):
    b0 = timer()
    root0, root, nam, desc, builder, ver, wshd, _, params, ts, dtb, dte, dftem = __lumpedCollect(ins,  "=== Raven Blended builder ===")

    # print("\n\n=== Writing control files..")
    buildBlended.buildLumped(root, nam, desc, builder, ver, wshd, params, ts, dtb, dte)
    ...
    if flg.calibrationmode: 
        # print('\n copying Ostrich templates..')
        ostrich_Blended.writeDDS(root0 + nam + ins.sfx + "_CALIB\\", nam, 4000)
